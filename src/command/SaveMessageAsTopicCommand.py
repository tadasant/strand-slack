...
class SaveMessageAsTopicCommand(Command):
    def __init__(self, slack_client_wrapper, core_api_client_wrapper, slack_team_id,
    ...
    def execute(self):
        ...
        discussion = self.core_api_client_wrapper.create_discussion(topic_id=topic.id)

        self.core_api_client_wrapper.create_message(text=slack_message.text,
                                                    discussion_id=discussion.id,
                                                    author_id=topic.original_poster.id,
                                                    time=slack_message.time)

        self.core_api_client_wrapper.close_discussion(discussion_id=discussion.id)
