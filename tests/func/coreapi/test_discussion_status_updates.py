...
class TestDiscussionStatusUpdates(TestFunction):
    ...
    def test_post_unauthorized(self):
        ...
        target_url = url_for(endpoint='coreapi.stalediscussionstatusresource')

        # Stale endpoint
        response = self.client.post(path=target_url, headers=headers, data=json.dumps(self.default_payload))
        assert response.status_code == HTTPStatus.UNAUTHORIZED

        target_url = url_for(endpoint='coreapi.closeddiscussionstatusresource')

        # Closed endpoint
        response = self.client.post(path=target_url, headers=headers, data=json.dumps(self.default_payload))
        assert response.status_code == HTTPStatus.UNAUTHORIZED

    def test_post_valid_stale(self, slack_agent_repository, slack_client_class, core_api_client, slack_client, mocker):
        ...
        def wait_condition():
            return core_api_client.mutate.call_count == 1 and slack_client_class.api_call.call_count >= 1

        outcome = wait_until(condition=wait_condition)
        assert outcome, 'Expected core_api_client to have 1 call and slack_client to have 1+ calls'

        assert 200 <= response.status_code <= 300
        assert 'markDiscussionAsPendingClosedFromSlack' in core_api_client.mutate.call_args_list[0][1][
    ...
    def test_post_valid_closed(self, slack_client_class, mocker, slack_agent_repository, core_api_client, slack_client):
        self.add_slack_agent_to_repository(slack_agent_repository=slack_agent_repository,
                                           slack_team_id=self.fake_discussion_status_request.slack_team_id)
        discussion_channel_id = self.start_discussion_on_channel(
            slack_team_id=self.fake_discussion_status_request.slack_team_id,
