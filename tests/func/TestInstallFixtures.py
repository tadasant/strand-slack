...
class TestInstallFixtures:
    ...
    def slack_oauth_access_response(self, slack_oauth_access_response_factory) -> NamedTuple:
        """
            Set TestSlackClient to contain a random slack oauth response.
            Yield namedtuple(code, slack_oauth_response)
        """
        response = namedtuple('response', 'code slack_oauth_response')
        # store faker values in the test Slack client repo w/ random code
        yield response(code='12345', slack_oauth_response=slack_oauth_access_response_factory)
        # clear the values
        pass
